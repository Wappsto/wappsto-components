...
function mapStateToProps(state, componentProps){
  return {
    request: getRequest(state, "/state/" + componentProps.state.meta.id, "PATCH")
  }
}
...
export class State extends Component {
  constructor(props){
    ...
    this.state = {
      input: (this.props.request && this.props.request.status === "pending" && this.props.request.body.data) || this.props.state.data
    }
  }
  ...
  updateState(data){
    ...
    if(!this.isUpdating()){
      ...
      this.props.makeRequest("PATCH", "/state/" + this.props.state.meta.id, { data, timestamp });
    }
  }

  isUpdating(){
    return this.props.request && this.props.request.status === "pending" ? true : false;
  }
}
